...
class ModelSelection(object):
    ...
    def start_models(self):
        """ Start models in separate Thread
        """
        try:

            model_threads = [Thread(target=self.init_model, args=(id))
                             for id in range(len(self.modelIds))]
            # daemonize and start
            ...
            print "Shutting down all models ASAP".format(self.model_name)
            raise
    ...
    def run(self):
        ...
        """
        try:
            models_thread = Thread(target=self.start_models)
