...
def dimension(feature_list):
    """
    Return the number of dimensiona a given set of features will be
    :feature_list: list of features to consider.
    :return: an integer for the dimension of the features.
    """
    feature_list = [f.lower() for f in feature_list]

    dim = 0
    if '<candidate_length>' in feature_list or '<all>' in feature_list:
        dim += 1
    elif '<user_length>' in feature_list or '<all>' in feature_list:
        dim += 1
